...
- [x] **Task 7:** HMAC signer — implement optional HMAC-SHA256 signing and verification with timing-safe comparison; add tests — scope: `python`, complexity: `medium`
- [x] **Task 8:** JSONL file store — implement append-only JSONL storage with 0o600 permissions, file recovery on init, corrupt line handling; add tests — scope: `python`, complexity: `large`
- [x] **Task 9:** Public API exports — wire up __init__.py re-exports, ensure `make all` passes cleanly — scope: `python`, complexity: `small`
...
- [x] **Task 12:** Store interface and memory store (TS mirror) — mirror Task 3: TrailStore interface and MemoryStore; tests — scope: `typescript`, complexity: `medium`
...
- [x] **Task 17:** Public API exports (TS mirror) — wire up index.ts re-exports, ensure `npm run all` passes cleanly — scope: `typescript`, complexity: `small`
