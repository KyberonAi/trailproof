...
<img src="/images/trailproof-flow.svg" alt="Trailproof pipeline: Your App → Event Builder (validate + envelope) → Hash Chain Engine (SHA-256) → Optional HMAC Signer → Append-Only Store (Memory or JSONL) → Verify + Query" style={{ width: '100%', height: 'auto' }} />
